...
.stTextInput>div>div>input,.stSlider label
...
.streamlit-expanderHeader{background:#1e293b !important;color:#93c5fd !important;
  border-radius:10px !important;font-weight:600 !important}
.streamlit-expanderContent{background:#1e293b !important;border-radius:0 0 10px 10px !important}
.stAlert{background:#1e293b !important;border-color:#334155 !important;border-radius:10px !important}
...
[data-testid="stMetricLabel"]{color:#94a3b8;font-size:.78rem;font-weight:500}
[data-testid="stMetricValue"]{color:#e2e8f0;font-weight:700}
[data-testid="stMetricDelta"]{font-size:.8rem}

/* ── Tableau ── */
.stDataFrame{background:#1e293b;border-radius:10px;overflow:hidden}
...
.stButton>button{
  ...
  padding:.6rem 1.5rem !important;width:100% !important;
...
/* ── Onglets ── */
[data-baseweb="tab-list"]{background:#1e293b !important;border-radius:10px !important;padding:4px !important}
[data-baseweb="tab"]{border-radius:8px !important;font-weight:500 !important;color:#94a3b8 !important}
[aria-selected="true"]{background:#1d4ed8 !important;color:white !important}

/* ── Barre de progression wizard ── */
.step-bar{display:flex;align-items:center;gap:0;margin:0 0 2rem;overflow-x:auto;padding-bottom:.3rem}
.step-item{display:flex;align-items:center;gap:.4rem;min-width:0}
.step-dot{width:30px;height:30px;border-radius:50%;display:flex;align-items:center;
  justify-content:center;font-size:12px;font-weight:700;flex-shrink:0;
  transition:all .3s}
.step-dot.done{background:linear-gradient(135deg,#1d4ed8,#2563eb);color:white;
  box-shadow:0 2px 8px rgba(37,99,235,0.4)}
.step-dot.current{background:linear-gradient(135deg,#3b82f6,#60a5fa);color:white;
  box-shadow:0 0 0 3px #1e3a5f,0 2px 12px rgba(96,165,250,0.4)}
.step-dot.todo{background:#1e293b;color:#475569;border:1px solid #334155}
.step-label{font-size:11px;color:#94a3b8;white-space:nowrap;overflow:hidden;
  text-overflow:ellipsis;max-width:80px}
.step-label.current{color:#93c5fd;font-weight:700}
.step-sep{width:24px;height:2px;background:linear-gradient(90deg,#334155,#1e293b);
...
.card{
  ...
  border-radius:14px;padding:1.4rem;margin:.5rem 0;
  box-shadow:0 4px 16px rgba(0,0,0,0.25);
  transition:box-shadow .2s}
.card:hover{box-shadow:0 6px 24px rgba(0,0,0,0.4)}
.card h4{color:#93c5fd;margin:0 0 .8rem;font-size:1rem;font-weight:600;
  letter-spacing:.3px}
...
.kpi{
  ...
  box-shadow:0 4px 16px rgba(0,0,0,0.25)}
.kpi-val{font-size:1.6rem;font-weight:700;letter-spacing:-.5px}
.kpi-lbl{font-size:.72rem;color:#94a3b8;margin-top:.3rem;font-weight:500;
  text-transform:uppercase;letter-spacing:.5px}

/* ── Verdicts ── */
.verdict-good{background:linear-gradient(135deg,#14532d,#166534);
  border-left:4px solid #22c55e;border-radius:0 10px 10px 0;
  padding:1rem 1.2rem;margin:.7rem 0;
  box-shadow:0 2px 12px rgba(34,197,94,0.15)}
.verdict-neutral{background:linear-gradient(135deg,#1e293b,#162032);
  border-left:4px solid #3b82f6;border-radius:0 10px 10px 0;
  padding:1rem 1.2rem;margin:.7rem 0}

/* ── Conseils ── */
.conseil{background:linear-gradient(135deg,#1c1a09,#22190a);
  border:1px solid #854d0e;border-radius:10px;
  padding:.9rem 1rem;margin:.4rem 0;
  box-shadow:0 2px 8px rgba(133,77,14,0.15)}
.src{background:#1e293b;border-radius:8px;padding:.4rem .9rem;
  font-size:.73rem;color:#64748b;margin:.4rem 0;border:1px solid #1e3a5f}
.highlight{background:linear-gradient(135deg,#1e3a5f,#172554);
  border:1px solid #1d4ed8;border-radius:10px;
  padding:.9rem 1.1rem;margin:.4rem 0;
  box-shadow:0 2px 12px rgba(29,78,216,0.2)}

/* ── Frais reels ── */
.fr-row{display:grid;grid-template-columns:1fr 1fr;gap:.5rem;margin:.3rem 0}
.fr-item{background:linear-gradient(135deg,#1e293b,#162032);
  border-radius:8px;padding:.6rem .9rem;border:1px solid #334155}
.fr-item .lbl{font-size:.75rem;color:#94a3b8;font-weight:500}
.fr-item .val{font-size:.95rem;font-weight:700;color:#e2e8f0}
.compare-bar{background:linear-gradient(135deg,#1e293b,#162032);
  border-radius:10px;padding:1.1rem;margin:.5rem 0;
  border:1px solid rgba(51,65,85,0.8)}

/* ── Masquer code brut et elements parasites ── */
...
<div style="text-align:center;padding:2rem 1rem 1rem">
  <div style="display:inline-block;background:linear-gradient(135deg,#1d4ed8,#3b82f6);
    border-radius:18px;padding:.7rem 1.2rem;margin-bottom:.8rem;
    box-shadow:0 4px 20px rgba(59,130,246,0.4)">
    <span style="font-size:2.2rem">🧮</span>
  </div>
  <h1 style="font-size:2.3rem;font-weight:800;color:#f1f5f9;margin:.4rem 0;
  ...
  <p style="font-size:1rem;color:#94a3b8;margin:.5rem auto 0;max-width:560px;line-height:1.7">
    Calculez votre impôt, optimisez votre situation et comparez vos scénarios
    — en quelques minutes, gratuitement, selon le barème officiel DGFiP.
  </p>
</div>
""", unsafe_allow_html=True)
    ...
    for col, ico, titre, sous in zip(
        [b1, b2, b3, b4],
        ["✅", "🔒", "⚡", "📄"],
        ["Barème DGFiP officiel", "100 % confidentiel", "Résultat immédiat", "Export PDF"],
        ...
        with col:
            st.markdown(f"""<div style="background:linear-gradient(135deg,#1e293b,#162032);
              ...
              <div style="color:#e2e8f0;font-weight:700;font-size:.82rem;margin:.25rem 0">{titre}</div>
              <div style="color:#64748b;font-size:.72rem">{sous}</div>
            </div>""", unsafe_allow_html=True)

    st.markdown("<div style='height:1.8rem'></div>", unsafe_allow_html=True)
    st.markdown("""<h2 style="text-align:center;color:#93c5fd;font-size:1.1rem;
      font-weight:700;margin-bottom:1rem;text-transform:uppercase;letter-spacing:.8px">
    ...
    with c1:
        st.markdown("""<div style="background:linear-gradient(160deg,#1e293b,#1e3a5f);
          border:1px solid rgba(59,130,246,0.35);border-radius:16px;
          padding:1.5rem 1.3rem 1rem;min-height:220px;
          box-shadow:0 4px 20px rgba(0,0,0,0.3)">
          <div style="font-size:1.8rem;margin-bottom:.4rem">🏠</div>
          <h3 style="color:#93c5fd;font-size:1.05rem;font-weight:700;margin:0 0 .5rem">
            Mon foyer fiscal</h3>
          <p style="color:#94a3b8;font-size:.84rem;line-height:1.6;margin:0 0 .9rem">
            Calculez l'impôt de votre foyer avec toutes les déductions, crédits
            et optimisations personnalisées.</p>
          <div style="display:flex;flex-wrap:wrap;gap:.3rem">
            <span style="background:#1d4ed822;color:#93c5fd;border:1px solid #1d4ed855;
              border-radius:20px;padding:.1rem .55rem;font-size:.72rem">Salaires</span>
            <span style="background:#1d4ed822;color:#93c5fd;border:1px solid #1d4ed855;
              border-radius:20px;padding:.1rem .55rem;font-size:.72rem">Pensions</span>
            <span style="background:#1d4ed822;color:#93c5fd;border:1px solid #1d4ed855;
              border-radius:20px;padding:.1rem .55rem;font-size:.72rem">PER</span>
            <span style="background:#1d4ed822;color:#93c5fd;border:1px solid #1d4ed855;
              border-radius:20px;padding:.1rem .55rem;font-size:.72rem">Frais réels</span>
          </div>
        </div>""", unsafe_allow_html=True)
        if st.button("🏠  Simuler mon foyer  →", key="btn_foyer", use_container_width=True):
            SS.mode = "foyer"; SS.data = {}; next_step()

    with c2:
        st.markdown("""<div style="background:linear-gradient(160deg,#1e293b,#1a2e1a);
          border:1px solid rgba(34,197,94,0.35);border-radius:16px;
          padding:1.5rem 1.3rem 1rem;min-height:220px;
          box-shadow:0 4px 20px rgba(0,0,0,0.3)">
          <div style="font-size:1.8rem;margin-bottom:.4rem">🎓</div>
          <h3 style="color:#86efac;font-size:1.05rem;font-weight:700;margin:0 0 .5rem">
            Enfant majeur : rattacher ou non ?</h3>
          <p style="color:#94a3b8;font-size:.84rem;line-height:1.6;margin:0 0 .9rem">
            Comparez les deux scénarios fiscaux et choisissez la meilleure option
            pour votre famille.</p>
          <div style="display:flex;flex-wrap:wrap;gap:.3rem">
            <span style="background:#16532d22;color:#86efac;border:1px solid #16532d55;
              border-radius:20px;padding:.1rem .55rem;font-size:.72rem">Rattachement</span>
            <span style="background:#16532d22;color:#86efac;border:1px solid #16532d55;
              border-radius:20px;padding:.1rem .55rem;font-size:.72rem">Foyer indép.</span>
            <span style="background:#16532d22;color:#86efac;border:1px solid #16532d55;
              border-radius:20px;padding:.1rem .55rem;font-size:.72rem">Pension alim.</span>
          </div>
        </div>""", unsafe_allow_html=True)
        if st.button("🎓  Comparer les scénarios  →", key="btn_comp", use_container_width=True):
            SS.mode = "comparaison"; SS.data = {}; next_step()

    with c3:
        st.markdown("""<div style="background:linear-gradient(160deg,#1e293b,#2d1a0e);
          border:1px solid rgba(251,146,60,0.35);border-radius:16px;
          padding:1.5rem 1.3rem 1rem;min-height:220px;
          box-shadow:0 4px 20px rgba(0,0,0,0.3)">
          <div style="font-size:1.8rem;margin-bottom:.4rem">💼</div>
          <h3 style="color:#fdba74;font-size:1.05rem;font-weight:700;margin:0 0 .5rem">
            Revenus de dirigeant</h3>
          <p style="color:#94a3b8;font-size:.84rem;line-height:1.6;margin:0 0 .9rem">
            IS, dividendes, TNS, LMNP, SCI... Optimisez votre rémunération
            selon votre statut.</p>
          <div style="display:flex;flex-wrap:wrap;gap:.3rem">
            <span style="background:#7c2d0d22;color:#fdba74;border:1px solid #7c2d0d55;
              border-radius:20px;padding:.1rem .55rem;font-size:.72rem">IS / SARL</span>
            <span style="background:#7c2d0d22;color:#fdba74;border:1px solid #7c2d0d55;
              border-radius:20px;padding:.1rem .55rem;font-size:.72rem">BIC / BNC</span>
            <span style="background:#7c2d0d22;color:#fdba74;border:1px solid #7c2d0d55;
              border-radius:20px;padding:.1rem .55rem;font-size:.72rem">LMNP / SCI</span>
            <span style="background:#7c2d0d22;color:#fdba74;border:1px solid #7c2d0d55;
              border-radius:20px;padding:.1rem .55rem;font-size:.72rem">TNS</span>
          </div>
        </div>""", unsafe_allow_html=True)
        if st.button("💼  Simuler mes revenus pro  →", key="btn_dir", use_container_width=True):
            SS.mode = "dirigeant"; SS.data = {}; next_step()

    # ── Section fonctionnalités ───────────────────────────────────
    st.markdown("<div style='height:1.5rem'></div>", unsafe_allow_html=True)
    st.markdown("""<div style="background:linear-gradient(135deg,#1e293b,#162032);
      border:1px solid #334155;border-radius:14px;padding:1.3rem 1.6rem">
      <h3 style="color:#93c5fd;font-size:.9rem;font-weight:700;margin:0 0 .9rem;
        text-transform:uppercase;letter-spacing:.5px">Ce que vous obtenez</h3>
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:.7rem">
        <div style="font-size:.82rem">✅ <b style="color:#e2e8f0">Calcul officiel DGFiP</b>
          <br><span style="color:#64748b">Barème 2026, décote, quotient familial</span></div>
        <div style="font-size:.82rem">✅ <b style="color:#e2e8f0">Frais réels automatiques</b>
          <br><span style="color:#64748b">Barème km, repas, télétravail</span></div>
        <div style="font-size:.82rem">✅ <b style="color:#e2e8f0">Optimisations personnalisées</b>
          <br><span style="color:#64748b">PER, dons, emploi domicile, heures sup</span></div>
        <div style="font-size:.82rem">✅ <b style="color:#e2e8f0">Comparaison enfant majeur</b>
          <br><span style="color:#64748b">Rattachement vs indép. avec verdict</span></div>
        <div style="font-size:.82rem">✅ <b style="color:#e2e8f0">Guide des cases fiscales</b>
          <br><span style="color:#64748b">Cases 2042 avec montants à reporter</span></div>
        <div style="font-size:.82rem">✅ <b style="color:#e2e8f0">Rapport PDF professionnel</b>
          <br><span style="color:#64748b">Bilan complet téléchargeable</span></div>
      </div>
    </div>""", unsafe_allow_html=True)

    st.markdown("<div style='height:.6rem'></div>", unsafe_allow_html=True)
    st.markdown("""<div class="src" style="text-align:center">
      Simulation à titre informatif — Brochure DGFiP 2025 (revenus 2024) —
      Consultez impots.gouv.fr ou un expert-comptable pour votre situation.
    </div>""", unsafe_allow_html=True)
    ...
    # ── Étape 2 : Revenus Déclarant 1 ─────────────────────────────
        ...
        c1, c2 = st.columns(2)
        with c1:
            sal1 = st.number_input("Salaires / Traitements EUR (case 1AJ)",
