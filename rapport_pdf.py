...
class GenererRapportPDF(FPDF):
    ...
    # ─── Titre de catégorie de cases ──────────────────────────────────
    def _case_category(self, titre, sous_titre=''):
        """Affiche un en-tête de catégorie stylisé avec sous-titre optionnel."""
        self.ln(2)
        self.set_fill_color(255, 247, 230)
        self.set_draw_color(*self.ORANGE)
        self.set_font('Helvetica', 'B', 9)
        self.set_text_color(*self.ORANGE)
        self.cell(0, 7, _s(f'  {titre}'), fill=True, border='L', ln=True)
        self.set_draw_color(200, 200, 200)
        if sous_titre:
            self.set_font('Helvetica', 'I', 7.5)
            self.set_text_color(*self.GRIS)
            self.cell(0, 4.5, _s(f'  {sous_titre}'), ln=True)
        self.set_text_color(*self.NOIR)
        self.ln(1.5)
    ...
    def page_comparaison(self, comparaison, profil_enfant):
        ...
        self.set_font('Helvetica', 'B', 8.5)
        self.set_text_color(*self.BLEU_CLAIR)
        _pen_label = (f' | Pension versee : {fr(eb.get("pension_recue",0))} EUR'
                      if eb.get('pension_recue', 0) > 0 else '')
        self.cell(0, 6,
        ...
        self.ligne('  Revenu Net Imposable parents',
                   f'{fr(pb.get("revenu_imposable", 0))} EUR')
        if pb.get('pension_versee', 0) > 0:
            self.ligne('  Pension versee deductible (case 6GI)',
                       f'-{fr(pb["pension_versee"])} EUR')
        ...
        if eb.get('pension_recue', 0) > 0:
            self.ligne('  Pension recue imposable (case 1AS)',
                       f'{fr(eb["pension_recue"])} EUR')
