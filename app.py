...
STEPS_DIR = [
    "Accueil", "Statuts", "Informations", "Autres revenus",
    "Resultats", "Rapport PDF"
]

# ─── Données statuts dirigeant ────────────────────────────────────
_STATUTS_DIR = {
    'gerant_maj_sarl': {
        'label': '\U0001f3e2 Gérant majoritaire SARL (TNS)',
        'desc': 'Détient > 50% du capital — charges ~45%',
        'champs': ['remuneration', 'benefice', 'dividendes'],
        'moteur_key': 'gerant_majoritaire_sarl',
    },
    'president_sas': {
        'label': '\U0001f3db\ufe0f Président SAS / SASU',
        'desc': 'Assimilé salarié — charges ~75%',
        'champs': ['remuneration', 'benefice', 'dividendes'],
        'moteur_key': 'president_sas',
    },
    'gerant_min_sarl': {
        'label': '\U0001f3e2 Gérant minoritaire / égalitaire SARL',
        'desc': 'Assimilé salarié (\u2264 50% capital)',
        'champs': ['remuneration', 'dividendes'],
        'moteur_key': 'president_sas',
    },
    'ae_bic': {
        'label': '\U0001f6d2 Auto-entrepreneur BIC (commerce/vente)',
        'desc': 'CA plafonné 188 700 EUR — abattement 50/71%',
        'champs': ['ca', 'type_activite_bic'],
        'moteur_key': 'ae_bic',
    },
    'ae_bnc': {
        'label': '\U0001f4cb Auto-entrepreneur BNC (libéral/services)',
        'desc': 'Recettes plafonnées 77 700 EUR — abattement 34%',
        'champs': ['recettes'],
        'moteur_key': 'ae_bnc',
    },
    'tns_reel': {
        'label': '\U0001f4bc TNS réel (EI, EURL, SNC)',
        'desc': 'Imposition sur bénéfice réel',
        'champs': ['benefice'],
        'moteur_key': 'tns_reel',
    },
    'sci_is': {
        'label': "\U0001f3e0 SCI à l'IS (gérant)",
        'desc': "Société civile immobilière à l'IS",
        'champs': ['resultat_sci', 'quote_part_sci', 'dividendes_sci'],
        'moteur_key': None,
    },
}

_AUTRES_REV_DIR = {
    'lmnp':       {'label': '\U0001f3e0 LMNP — Loueur Meublé Non Professionnel', 'desc': 'Recettes < 23 000 EUR ou < 50% revenus'},
    'lmp':        {'label': '\U0001f3e0 LMP — Loueur Meublé Professionnel',      'desc': 'Recettes > 23 000 EUR ET > autres revenus pro'},
    'sci_ir':     {'label': "\U0001f3e2 SCI à l'IR (revenus fonciers)",           'desc': 'Transparence fiscale — quote-part résultat'},
    'foncier_nu': {'label': '\U0001f3d8\ufe0f Revenus fonciers (location nue)',  'desc': 'Micro-foncier ou régime réel (2044)'},
}
...
def _calculer_dir(d, SS, dm):
    """Calcule tous les résultats pour les statuts sélectionnés + autres revenus."""
    statuts       = SS.dir_statuts
    donnees       = SS.dir_donnees
    nb_map        = SS.dir_nb_ent
    autres_actifs = SS.dir_autres_actifs
    autres_d      = SS.dir_autres_donnees
    tmi           = d.get('tmi_dir', 30)

    res = {'statuts': {}, 'autres_revenus': {}, 'total_imposable': 0,
           'total_cotisations': 0, 'tmi': tmi}
    total_imp = total_cot = 0

    for sk in statuts:
        sinfo    = _STATUTS_DIR[sk]
        mk       = sinfo['moteur_key']
        ents     = donnees.get(sk, [])
        ents_res = []
        for i, ent in enumerate(ents):
            r = {'idx': i + 1}
            try:
                if sk in ('gerant_maj_sarl', 'president_sas', 'gerant_min_sarl'):
                    rem = ent.get('remuneration', 0)
                    ben = ent.get('benefice', 0)
                    div = ent.get('dividendes', 0)
                    if ben > 0:
                        r['is'] = dm.calculer_is(ben)
                    if rem > 0:
                        r['gerance']     = dm.calculer_gerance(rem)
                        r['cotisations'] = dm.calculer_cotisations_tns(rem, mk)
                        total_imp += r['gerance']['imposable']
                        total_cot += r['cotisations']['cotisations']
                    if div > 0:
                        r['dividendes_calcul'] = dm.calculer_dividendes(div, tmi)
                        total_imp += div
                    if rem > 0 and ben > 0:
                        try:
                            r['sim_rem_div'] = dm.simuler_remuneration_vs_dividendes(ben, rem, tmi)
                        except Exception:
                            pass
                    if ben > 0 or rem > 0:
                        try:
                            r['optimisations'] = dm.optimisation_fiscale({
                                'benefice': max(ben, 0), 'remuneration': rem,
                                'tmi': tmi, 'statut_actuel': mk,
                                'ca': max(ben, rem) * 3, 'type_activite': 'services'
                            })
                        except Exception:
                            r['optimisations'] = []
                elif sk == 'ae_bic':
                    ca   = ent.get('ca', 0)
                    tact = ent.get('type_activite_bic', 'services')
                    if ca > 0:
                        r['bic_micro'] = dm.calculer_bic_micro(ca, tact)
                        total_imp += r['bic_micro'].get('benefice_imposable', 0)
                        total_cot += ca * 0.129
                elif sk == 'ae_bnc':
                    rec = ent.get('recettes', 0)
                    if rec > 0:
                        r['bnc_micro'] = dm.calculer_bnc_micro(rec)
                        total_imp += r['bnc_micro'].get('benefice_imposable', 0)
                        total_cot += rec * 0.214
                elif sk == 'tns_reel':
                    ben = ent.get('benefice', 0)
                    if ben > 0:
                        r['cotisations'] = dm.calculer_cotisations_tns(ben, 'tns_reel')
                        total_imp += ben
                        total_cot += r['cotisations']['cotisations']
                elif sk == 'sci_is':
                    rs  = ent.get('resultat_sci', 0)
                    qp  = ent.get('quote_part_sci', 100) / 100
                    div = ent.get('dividendes_sci', 0)
                    if rs > 0:
                        r['sci_calcul'] = dm.calculer_sci_is(rs, qp)
                    if div > 0:
                        r['dividendes_calcul'] = dm.calculer_dividendes(div, tmi)
                        total_imp += div
                ...
                r['erreur'] = str(_e)
            ents_res.append(r)
        res['statuts'][sk] = ents_res

    for ak in autres_actifs:
        ad = autres_d.get(ak, {})
        ar = {}
        try:
            if ak == 'lmnp':
                rec = ad.get('recettes', 0)
                if rec > 0:
                    if ad.get('regime', 'micro') == 'micro':
                        ar['calcul'] = dm.calculer_lmnp(rec, ad.get('classe', False), 'micro')
                    else:
                        ar['calcul'] = dm.calculer_lmnp(rec, False, 'reel',
                                         ad.get('charges', 0), ad.get('amortissements', 0))
                    total_imp += ar['calcul'].get('imposable', 0)
            elif ak == 'lmp':
                rec = ad.get('recettes', 0)
                if rec > 0:
                    ar['calcul'] = dm.calculer_lmp(rec, ad.get('charges', 0),
                                     ad.get('amortissements', 0), ad.get('autres_rev_foyer', 0))
                    total_imp += ar['calcul'].get('imposable', 0)
            elif ak == 'sci_ir':
                qp = ad.get('quote_part', 0)
                ar['calcul'] = dm.calculer_sci_ir(qp)
                total_imp += ar['calcul'].get('imposable', 0)
            elif ak == 'foncier_nu':
                if ad.get('regime', 'micro') == 'micro':
                    rev = ad.get('revenus_bruts', 0)
                    if rev > 0:
                        ar['calcul'] = dm.calculer_foncier_micro(rev)
                        total_imp += ar['calcul'].get('imposable', 0)
                else:
                    rev = ad.get('revenus_bruts', 0)
                    if rev > 0:
                        ar['calcul'] = dm.calculer_foncier_reel(
                            rev, ad.get('interets', 0), ad.get('charges', 0),
                            ad.get('travaux', 0), ad.get('taxe_fonciere', 0), 0)
                        total_imp += ar['calcul'].get('imposable', 0)
        except Exception as _e:
            ar['erreur'] = str(_e)
        res['autres_revenus'][ak] = ar

    res['total_imposable']   = round(total_imp)
    res['total_cotisations'] = round(total_cot)
    return res
...
<div style="text-align:center;padding:2.5rem 1rem 1.5rem">
  ...
  <p style="font-size:1.05rem;color:#94a3b8;margin:.6rem auto 0;max-width:580px;
    ...
    if SS.step == 1:
        ...
        _OPTS_SIT = ["Celibataire / Divorce(e)", "Marie(e) ou Pacse(e)", "Veuf(ve)"]
        _SIT_TO_LABEL = {"Celibataire / Divorce(e)": "Celibataire / Divorce(e)",
                         "Marie(e) / Pacse(e)": "Marie(e) ou Pacse(e)",
                         "Veuf(ve)": "Veuf(ve)"}
        _sit_label = _SIT_TO_LABEL.get(d.get('situation', "Celibataire / Divorce(e)"), "Celibataire / Divorce(e)")
        sit = st.radio("Vous etes :", _OPTS_SIT, key="sit_radio", index=_OPTS_SIT.index(_sit_label))
        ...
        for i in range(int(nb_rat)):
            ...
            with c2:
                ...
                etud_r = st.checkbox(f"Etudiant (exo. 5 406 EUR)", er.get('etudiant', False), key=f"etud_r_{i}")
                exo_r = min(sal_r, 5406) if etud_r else 0
                if etud_r and exo_r:
    ...
    # ── Étape 2 : Revenus Déclarant 1 ─────────────────────────────
        ...
        st.markdown("## Vos revenus 2025 — Declarant 1")
        ...
        st.info(
            "**Arrêt maladie en 2025 ?** Les IJ CPAM sont **imposables** et pré-remplies en case 1AJ. "
    ...
    # ── Étape 3 : Revenus D2 (si couple) ou Frais Pro (si seul) ────
        ...
        if d.get('situation') == "Marie(e) / Pacse(e)":
            st.markdown("## Revenus 2025 — Declarant 2 (conjoint / partenaire)")
            c1, c2 = st.columns(2)
            with c1:
    ...
    # ── Étape 4 : Frais professionnels ────────────────────────────
        ...
        with tabs_fr[0]:
            st.markdown("#### Frais de deplacement — bareme kilometrique 2025")
            ...
            with fa:
                ...
                km = st.number_input("Kilometres professionnels en 2025",
            ...
            with fb:
                if km > 0:
                    ...
                    <h4>Indemnites kilomestriques 2025</h4>
        ...
        with tabs_fr[1]:
            ...
            st.info("Vous pouvez deduire la difference entre le cout d'un repas pris a l'exterieur et la valeur d'un repas au domicile (5,35 EUR en 2025).")

            nb_rep = st.number_input("Nombre de repas pris a l'exterieur pour raisons professionnelles en 2025",
        ...
        with tabs_fr[2]:
            ...
            jours_tt = st.number_input("Jours de teletravail en 2025",
    ...
    # ── Étape 5 : Déductions ───────────────────────────────────────
        ...
        per = st.number_input(
            "Versements PER en 2025 EUR (cases 6NS / 6NT / 6NU)",
        ...
        pen_versee = st.number_input(
            ...
            help="Plafond legal 2025 : 6 855 EUR par enfant majeur hors foyer")
        if pen_versee > 6855:
            st.warning("Plafond de deductibilite : 6 855 EUR. L'excedent ne sera pas deductible.")
    ...
    # ── Étape 6 : Credits & Réductions ────────────────────────────
        ...
        with c_d2:
            dons_mayo = st.number_input("Dons Mayotte urgence 75 % EUR (cyclone Chido)", 0, 5000, int(d.get('dons_mayo',0)), 50, key="dmayo_w", help="Case 7UM — cyclone Chido 14/12/2024 au 17/05/2025")
        with c_d3:
        ...
        nb_enf6 = st.number_input("Nombre d'enfants de moins de 6 ans au 01/01/2025",
        ...
        with et3:
            ...
            with eg1:
                etudiant_enf = st.checkbox("Enfant etudiant (< 26 ans)",ed.get('etudiant',False),key="etudiant_enf_w",
                    help="Exoneration emploi etudiant : 5 406 EUR max (art. 81 bis CGI)")
                if etudiant_enf:
                    ...
                    st.success(f"Exoneration : {fr(min(sal1_enf,5406))} EUR | Reduction scolarite (Scenario A) : {red_scol_est} EUR")
                else:
                    niveau_enf = list(moteur.SCOLARITE.keys())[-1]
            with eg2:
                pension_enf = st.slider("Pension que vous pourriez verser EUR",0,6855,int(ed.get('pension',0)),100,key="pension_enf_w",
                    help="Deductible chez vous (case 6GI) — Imposable chez l'enfant (case 1AS) — Plafond 6 855 EUR")
                st.caption("Chez vous : deductible **case 6GI**")
                st.caption("Chez l'enfant : imposable **case 1AS**")

        exo_etud_enf = min(sal1_enf, 5406) if etudiant_enf else 0
        d['enf_data'] = {
    ...
    # ── Étape 7 (foyer) ou 8 (comp) : Résultats ───────────────────
        ...
        profil = {
            ...
            'pension_alimentaire_versee': min(d.get('pen_versee',0), 6855),
        ...
        st.markdown('<div class="src">Bareme 2025 — DGFiP 2026 — Decote : 897 - 45,25 % x impot — QF plafonne 1 807 EUR/demi-part</div>',unsafe_allow_html=True)
        ...
        if SS.mode == "comparaison" and d.get('enf_data'):
            ...
            profil_enf = {
                ...
                'pension_recue': ed.get('pension', 0),
            }
            ...
            with cb_c:
                ...
                pension_b = pb.get('pension_versee', 0)
                lines_b_p = [
                    f"Parts parents : {pb['nb_parts']:.1f}",
                    f"RNI parents : {fr(pb['revenu_imposable'])} EUR",
                ]
                if pension_b > 0:
                    lines_b_p.append(f"Pension versee (6GI) : -{fr(pension_b)} EUR")
                lines_b_p.append(f"IR parents : {fr(pb['impot_net'])} EUR")
                ...
                pension_recue_b = eb.get('pension_recue', 0)
                if pension_recue_b > 0:
                    lines_b_e.append(f"Pension recue (1AS) : {fr(pension_recue_b)} EUR")
                lines_b_e += [
        ...
        # Conseil spécifique pension alimentaire (affiché uniquement en mode comparaison)
        if SS.mode == "comparaison" and d.get('enf_data'):
            st.markdown("---")
            st.markdown("""<div class="conseil">
            <b>💡 Conseil — Pension alimentaire (optionnel)</b><br>
            <span style="color:#fbbf24;font-size:.85rem">
            En Scénario B (foyer indépendant), vous pouvez <b>verser une pension alimentaire</b>
            à votre enfant (plafond <b>6 855 EUR/an, case 6GI</b>) :
            elle est <b>entièrement déductible</b> de votre revenu imposable et <b>imposable chez l'enfant</b> (case 1AS).
            Cela peut être avantageux si votre TMI est supérieur à celui de votre enfant.
            Utilisez le curseur "Pension" dans l'onglet Statut & Pension pour simuler cet effet.
...
# ═══════════════════════════════════════════════════════════════════
    ...
    for _k, _v in [
        ('dir_statuts', []), ('dir_nb_ent', {}), ('dir_donnees', {}),
        ('dir_autres_actifs', []), ('dir_autres_donnees', {}), ('dir_resultats', None),
    ]:
        if _k not in SS:
            SS[_k] = _v

    # ════════════════════════════════════════════════════════════
    # STEP 1 — SÉLECTION DES STATUTS
    # ════════════════════════════════════════════════════════════
    if SS.step == 1:
        st.markdown("## 👔 Vos statuts de dirigeant")
        st.markdown(
            "<div style='background:#1e293b;border:1px solid #334155;"
            "border-radius:10px;padding:.7rem 1rem;margin-bottom:1rem;"
            "color:#94a3b8;font-size:.86rem'>Cochez <b style='color:#e2e8f0'>un ou plusieurs statuts</b> "
            "que vous détenez. Vous pouvez cumuler plusieurs statuts.</div>",
            unsafe_allow_html=True
        )
        new_statuts = []
        for sk, sinfo in _STATUTS_DIR.items():
            c1, c2 = st.columns([1, 9])
            with c1:
                checked = st.checkbox("", value=(sk in SS.dir_statuts), key=f"dchk_{sk}")
            with c2:
                st.markdown(
                    f"<div style='padding:.35rem 0'>"
                    f"<b style='color:#e2e8f0'>{sinfo['label']}</b>"
                    f"<span style='color:#64748b;font-size:.8rem;margin-left:.6rem'>{sinfo['desc']}</span>"
                    f"</div>", unsafe_allow_html=True
                )
            if checked:
                new_statuts.append(sk)

        st.markdown("---")
        if new_statuts:
            st.markdown("### 🏢 Nombre d'entreprises par statut")
            nb_cols = min(len(new_statuts), 3)
            cols = st.columns(nb_cols)
            nb_ent = {}
            for i, sk in enumerate(new_statuts):
                with cols[i % nb_cols]:
                    lbl = _STATUTS_DIR[sk]['label'].split('(')[0].strip()
                    nb = st.number_input(
                        f"Nb entreprises\n**{lbl}**",
                        min_value=1, max_value=10,
                        value=SS.dir_nb_ent.get(sk, 1),
                        step=1, key=f"dnb_{sk}"
                    )
                    nb_ent[sk] = int(nb)
            st.markdown("---")
            c_tmi, _ = st.columns([1, 2])
            with c_tmi:
                tmi_val = st.select_slider(
                    "TMI estimé (%)", options=[0, 11, 30, 41, 45],
                    value=d.get('tmi_dir', 30), key="tmi_dir_w",
                    help="Tranche Marginale d'Imposition"
                )
                d['tmi_dir'] = tmi_val
            st.markdown("---")
            c1, _, c2 = st.columns([1, 3, 1])
            with c1:
                if st.button("← Précédent", key="dprev_1"):
                    SS.dir_statuts = new_statuts
                    SS.dir_nb_ent  = nb_ent
                    prev_step()
            with c2:
                if st.button("Suivant →", key="dnext_1", type="primary"):
                    SS.dir_statuts = new_statuts
                    SS.dir_nb_ent  = nb_ent
                    existing = SS.dir_donnees
                    for sk in new_statuts:
                        if sk not in existing:
                            existing[sk] = []
                        while len(existing[sk]) < nb_ent[sk]:
                            existing[sk].append({})
                        existing[sk] = existing[sk][:nb_ent[sk]]
                    SS.dir_donnees = existing
                    next_step()
        else:
            st.info("👆 Cochez au moins un statut pour continuer.")
            c1, _ = st.columns([1, 7])
            with c1:
                if st.button("← Précédent", key="dprev_1_empty"):
                    prev_step()

    # ════════════════════════════════════════════════════════════
    # STEP 2 — INFORMATIONS FINANCIÈRES PAR STATUT
    # ════════════════════════════════════════════════════════════
    elif SS.step == 2:
        st.markdown("## 💼 Informations financières par statut")
        statuts = SS.dir_statuts
        nb_map  = SS.dir_nb_ent
        donnees = SS.dir_donnees
        tmi     = d.get('tmi_dir', 30)
        if not statuts:
            st.warning("Aucun statut sélectionné. Retournez à l'étape précédente.")
            if st.button("← Retour", key="dprev_2_nostat"):
                prev_step()
        else:
            for sk in statuts:
                sinfo  = _STATUTS_DIR[sk]
                nb     = nb_map.get(sk, 1)
                champs = sinfo['champs']
                with st.expander(f"{sinfo['label']} — {nb} entreprise(s)", expanded=True):
                    if sk not in donnees:
                        donnees[sk] = [{} for _ in range(nb)]
                    for i in range(nb):
                        if i >= len(donnees[sk]):
                            donnees[sk].append({})
                        ent = donnees[sk][i]
                        ...
                        if 'remuneration' in champs:
                            v = st.number_input("Rémunération annuelle brute (€)",
                                0, 1_000_000, int(ent.get('remuneration', 0)), 500,
                                key=f"d_{sk}_{i}_rem",
                                help="Rémunération versée par la société")
                            donnees[sk][i]['remuneration'] = v
                        if 'benefice' in champs:
                            v = st.number_input("Bénéfice avant IS / résultat fiscal (€)",
                                0, 10_000_000, int(ent.get('benefice', 0)), 1000,
                                key=f"d_{sk}_{i}_ben",
                                help="Résultat fiscal avant IS")
                            donnees[sk][i]['benefice'] = v
                        if 'ca' in champs:
                            v = st.number_input("Chiffre d'affaires annuel (€)",
                                0, 500_000, int(ent.get('ca', 0)), 500,
                                key=f"d_{sk}_{i}_ca")
                            donnees[sk][i]['ca'] = v
                        if 'type_activite_bic' in champs:
                            opts = ["services", "vente", "mixte"]
                            fmt  = {
                                "services": "Prestations de services (abatt. 50%)",
                                "vente":    "Vente / Commerce (abatt. 71%)",
                                "mixte":    "Activité mixte (abatt. 50%)"
                            }
                            v = st.selectbox("Type d'activité BIC", opts,
                                index=opts.index(ent.get('type_activite_bic', 'services')),
                                format_func=lambda x: fmt[x],
                                key=f"d_{sk}_{i}_tbic")
                            donnees[sk][i]['type_activite_bic'] = v
                        if 'recettes' in champs:
                            v = st.number_input("Recettes annuelles (€)",
                                0, 300_000, int(ent.get('recettes', 0)), 500,
                                key=f"d_{sk}_{i}_rec")
                            donnees[sk][i]['recettes'] = v
                        if 'resultat_sci' in champs:
                            v = st.number_input("Résultat fiscal SCI (€)",
                                0, 5_000_000, int(ent.get('resultat_sci', 0)), 1000,
                                key=f"d_{sk}_{i}_sci_res")
                            donnees[sk][i]['resultat_sci'] = v
                        if 'quote_part_sci' in champs:
                            v = st.number_input("Votre quote-part dans la SCI (%)",
                                1, 100, int(ent.get('quote_part_sci', 100)), 1,
                                key=f"d_{sk}_{i}_sci_qp")
                            donnees[sk][i]['quote_part_sci'] = v
                        if 'dividendes' in champs:
                            v = st.number_input("Dividendes perçus ou envisagés (€)",
                                0, 5_000_000, int(ent.get('dividendes', 0)), 500,
                                key=f"d_{sk}_{i}_div",
                                help="Dividendes distribués depuis le bénéfice net après IS")
                            donnees[sk][i]['dividendes'] = v
                        if 'dividendes_sci' in champs:
                            v = st.number_input("Dividendes SCI distribués (€)",
                                0, 2_000_000, int(ent.get('dividendes_sci', 0)), 500,
                                key=f"d_{sk}_{i}_div_sci")
                            donnees[sk][i]['dividendes_sci'] = v
                        if nb > 1 and i < nb - 1:
                            st.markdown("---")
            SS.dir_donnees = donnees
            st.markdown("---")
            c1, _, c2 = st.columns([1, 3, 1])
            with c1:
                if st.button("← Précédent", key="dprev_2"):
                    SS.dir_donnees = donnees
                    prev_step()
            with c2:
                if st.button("Suivant →", key="dnext_2", type="primary"):
                    SS.dir_donnees = donnees
                    next_step()

    # ════════════════════════════════════════════════════════════
    # STEP 3 — AUTRES REVENUS (LMNP, LMP, SCI IR, FONCIER)
    # ════════════════════════════════════════════════════════════
    elif SS.step == 3:
        st.markdown("## 🏘️ Autres revenus complémentaires")
        st.markdown(
            "<div style='background:#1e293b;border:1px solid #334155;border-radius:10px;"
            "padding:.7rem 1rem;margin-bottom:1rem;color:#94a3b8;font-size:.86rem'>"
            "Possédez-vous d'autres sources de revenus ? "
            "Cochez ce qui s'applique <b style='color:#e2e8f0'>(optionnel)</b>.</div>",
            unsafe_allow_html=True
        )
        autres_actifs  = list(SS.dir_autres_actifs)
        autres_donnees = dict(SS.dir_autres_donnees)
        new_actifs = []
        for ak, ainfo in _AUTRES_REV_DIR.items():
            c1, c2 = st.columns([1, 9])
            with c1:
                chk = st.checkbox("", value=(ak in autres_actifs), key=f"dachk_{ak}")
            with c2:
                st.markdown(
                    f"<div style='padding:.35rem 0'><b style='color:#e2e8f0'>{ainfo['label']}</b>"
                    f"<span style='color:#64748b;font-size:.8rem;margin-left:.6rem'>{ainfo['desc']}</span></div>",
                    unsafe_allow_html=True
                )
            if chk:
                new_actifs.append(ak)
        if new_actifs:
            st.markdown("---")
            st.markdown("### 📝 Détails des revenus complémentaires")
            for ak in new_actifs:
                if ak not in autres_donnees:
                    autres_donnees[ak] = {}
                ad = autres_donnees[ak]
                if ak == 'lmnp':
                    with st.expander("🏠 LMNP — Détails", expanded=True):
                        c1, c2 = st.columns(2)
                        with c1:
                            rec = st.number_input("Recettes locatives (€)", 0, 300_000,
                                int(ad.get('recettes', 0)), 500, key="dalmnp_rec")
                            cl  = st.checkbox("Bien classé meublé de tourisme",
                                ad.get('classe', False), key="dalmnp_cl")
                            ad['recettes'] = rec; ad['classe'] = cl
                        with c2:
                            reg = st.radio("Régime fiscal", ["Micro-BIC", "Réel"],
                                index=0 if ad.get('regime', 'micro') == 'micro' else 1,
                                key="dalmnp_reg")
                            ad['regime'] = 'micro' if reg == "Micro-BIC" else 'reel'
                        if ad['regime'] == 'reel':
                            c1, c2 = st.columns(2)
                            with c1:
                                ad['charges'] = st.number_input("Charges déductibles (€)", 0, 200_000,
                                    int(ad.get('charges', 0)), 500, key="dalmnp_ch")
                            with c2:
                                ad['amortissements'] = st.number_input("Amortissements (€)", 0, 200_000,
                                    int(ad.get('amortissements', 0)), 500, key="dalmnp_am")
                elif ak == 'lmp':
                    with st.expander("🏠 LMP — Détails", expanded=True):
                        c1, c2 = st.columns(2)
                        with c1:
                            ad['recettes'] = st.number_input("Recettes locatives (€)", 0, 2_000_000,
                                int(ad.get('recettes', 0)), 500, key="dalmp_rec")
                            ad['charges']  = st.number_input("Charges déductibles (€)", 0, 500_000,
                                int(ad.get('charges', 0)), 500, key="dalmp_ch")
                        with c2:
                            ad['amortissements']   = st.number_input("Amortissements (€)", 0, 500_000,
                                int(ad.get('amortissements', 0)), 500, key="dalmp_am")
                            ad['autres_rev_foyer'] = st.number_input("Autres revenus pro du foyer (€)", 0, 1_000_000,
                                int(ad.get('autres_rev_foyer', 0)), 1000, key="dalmp_arv",
                                help="Pour vérifier le seuil LMP")
                elif ak == 'sci_ir':
                    with st.expander("🏢 SCI à l'IR — Détails", expanded=True):
                        c1, c2 = st.columns(2)
                        with c1:
                            ad['quote_part'] = st.number_input(
                                "Quote-part du résultat (€ — négatif si déficit)", -200_000, 500_000,
                                int(ad.get('quote_part', 0)), 500, key="dasci_qp")
                        with c2:
                            st.info("ℹ️ Revenus fonciers transparents (décl. 2044).")
                elif ak == 'foncier_nu':
                    with st.expander("🏘️ Revenus fonciers nus — Détails", expanded=True):
                        reg = st.radio("Régime",
                            ["Micro-foncier (revenus < 15 000 €)", "Régime réel (déclaration 2044)"],
                            key="dafonc_reg")
                        ad['regime'] = 'micro' if 'Micro' in reg else 'reel'
                        if ad['regime'] == 'micro':
                            ad['revenus_bruts'] = st.number_input("Revenus bruts fonciers (€)", 0, 15_000,
                                int(ad.get('revenus_bruts', 0)), 200, key="dafonc_rb")
                        else:
                            c1, c2 = st.columns(2)
                            with c1:
                                ad['revenus_bruts'] = st.number_input("Revenus bruts (€)", 0, 500_000,
                                    int(ad.get('revenus_bruts', 0)), 500, key="dafonc_rev")
                                ad['interets']      = st.number_input("Intérêts d'emprunt (€)", 0, 200_000,
                                    int(ad.get('interets', 0)), 500, key="dafonc_int")
                            with c2:
                                ad['charges']       = st.number_input("Charges courantes (€)", 0, 100_000,
                                    int(ad.get('charges', 0)), 500, key="dafonc_ch")
                                ad['travaux']       = st.number_input("Travaux déductibles (€)", 0, 300_000,
                                    int(ad.get('travaux', 0)), 500, key="dafonc_trav")
                                ad['taxe_fonciere'] = st.number_input("Taxe foncière (€)", 0, 30_000,
                                    int(ad.get('taxe_fonciere', 0)), 100, key="dafonc_tf")
                autres_donnees[ak] = ad
        SS.dir_autres_actifs  = new_actifs
        SS.dir_autres_donnees = autres_donnees
        st.markdown("---")
        c1, _, c2 = st.columns([1, 3, 1])
        with c1:
            if st.button("← Précédent", key="dprev_3"):
                SS.dir_autres_actifs  = new_actifs
                SS.dir_autres_donnees = autres_donnees
                prev_step()
        with c2:
            if st.button("✅ Calculer les résultats", key="dnext_3", type="primary"):
                SS.dir_autres_actifs  = new_actifs
                SS.dir_autres_donnees = autres_donnees
                SS.dir_resultats      = _calculer_dir(d, SS, dm)
                next_step()
    ...
    # ════════════════════════════════════════════════════════════
        ...
        res = SS.dir_resultats
        tmi = d.get('tmi_dir', 30)
        if not res:
            st.error("Pas de résultats. Recommencez depuis le début.")
            if st.button("← Retour", key="dprev_4_err"):
                prev_step()
        else:
            st.markdown("## 📊 Bilan fiscal dirigeant")
            total_imp = res.get('total_imposable', 0)
            total_cot = res.get('total_cotisations', 0)
            ir_est    = int(total_imp * tmi / 100)
            c1, c2, c3 = st.columns(3)
            with c1:
                st.markdown(kpi_html("Revenus imposables IR", f"{fr(total_imp)} €"), unsafe_allow_html=True)
            with c2:
                st.markdown(kpi_html("Cotisations sociales est.", f"{fr(total_cot)} €", "#f59e0b"), unsafe_allow_html=True)
            with c3:
                st.markdown(kpi_html("IR estimé (TMI)", f"{fr(ir_est)} €", "#f87171"), unsafe_allow_html=True)
            st.markdown("---")

            for sk, ents_res in res.get('statuts', {}).items():
                if sk not in _STATUTS_DIR:
                    continue
                sinfo = _STATUTS_DIR[sk]
                with st.expander(f"📋 {sinfo['label']}", expanded=True):
                    for r in ents_res:
                        if len(ents_res) > 1:
                            st.markdown(f"**— Entreprise {r['idx']} —**")
                        if 'erreur' in r:
                            st.error(f"Erreur de calcul : {r['erreur']}")
                            continue
                        if 'is' in r:
                            is_d = r['is']
                            st.markdown("**🏦 Impôt sur les Sociétés**")
                            cc1, cc2, cc3 = st.columns(3)
                            cc1.metric("IS réduit 15%", f"{fr(is_d['is_reduit'])} €")
                            cc2.metric("IS normal 25%", f"{fr(is_d['is_normal'])} €")
                            cc3.metric("IS total",      f"{fr(is_d['is_total'])} €")
                            st.metric("Bénéfice net après IS", f"{fr(is_d['benefice_net'])} €")
                        if 'gerance' in r:
                            g = r['gerance']
                            st.markdown("**💼 Rémunération de gérance**")
                            cc1, cc2, cc3 = st.columns(3)
                            cc1.metric("Rémunération brute", f"{fr(g['remuneration'])} €")
                            cc2.metric("Abattement",          f"-{fr(g['abattement'])} €")
                            cc3.metric("Imposable IR",        f"{fr(g['imposable'])} €")
                            st.caption(f"IR estimé (TMI {tmi} %) : {fr(int(g['imposable']*tmi/100))} €")
                        if 'cotisations' in r:
                            cot = r['cotisations']
                            st.markdown("**🔄 Cotisations sociales**")
                            cc1, cc2, cc3 = st.columns(3)
                            cc1.metric("Taux charges",  f"{int(cot['taux_global']*100)} %")
                            cc2.metric("Cotisations",   f"{fr(cot['cotisations'])} €")
                            cc3.metric("Net avant IR",  f"{fr(cot['net_avant_ir'])} €")
                            det = "".join(
                                f"<div style='display:flex;justify-content:space-between;padding:.15rem 0'>"
                                f"<span style='color:#94a3b8;font-size:.8rem'>{k}</span>"
                                f"<b style='font-size:.8rem'>{v}</b></div>"
                                for k, v in cot['detail'].items()
                            )
                            with st.expander("Détail cotisations"):
                                st.markdown(det, unsafe_allow_html=True)
                        if 'bic_micro' in r:
                            b = r['bic_micro']
                            st.markdown(f"**📊 {b.get('type','BIC Micro')}**")
                            cc1, cc2 = st.columns(2)
                            cc1.metric("Chiffre d'affaires", f"{fr(b['ca'])} €")
                            cc2.metric("Bénéfice imposable",  f"{fr(b['benefice_imposable'])} €")
                            st.caption(f"IR estimé : {fr(int(b['benefice_imposable']*tmi/100))} €")
                            if not b.get('eligible', True):
                                st.warning("⚠️ CA dépasse le seuil micro. Passage au régime réel recommandé.")
                        if 'bnc_micro' in r:
                            b = r['bnc_micro']
                            st.markdown("**📋 BNC Micro (micro-BNC)**")
                            cc1, cc2 = st.columns(2)
                            cc1.metric("Recettes",            f"{fr(b['recettes'])} €")
                            cc2.metric("Bénéfice imposable",  f"{fr(b['benefice_imposable'])} €")
                            st.caption(f"Abattement 34% : -{fr(b.get('abattement_montant',0))} €  |  IR estimé : {fr(int(b['benefice_imposable']*tmi/100))} €")
                            if not b.get('eligible', True):
                                st.warning("⚠️ Recettes dépassent le seuil micro.")
                        if 'dividendes_calcul' in r:
                            div_d = r['dividendes_calcul']
                            st.markdown("**💰 Dividendes — PFU vs Barème**")
                            cc1, cc2 = st.columns(2)
                            pfu   = div_d['pfu']; bar = div_d['bareme']
                            meill = div_d['meilleur']; eco = div_d.get('economie', 0)
                            with cc1:
                                bc = "#3b82f6" if meill == "PFU" else "#334155"
                                st.markdown(f'''<div class="card" style="border-color:{bc}">
                                <b style="color:{bc}">{"✅ " if meill=="PFU" else ""}PFU 30 %</b><br>
                                IR {fr(pfu.get("ir",0))} € + PS {fr(pfu.get("ps",0))} € = <b>{fr(pfu["total_imposition"])} €</b><br>
                                Net perçu : <b>{fr(pfu["net_percu"])} €</b></div>''', unsafe_allow_html=True)
                            with cc2:
                                bc = "#4ade80" if meill == "Bareme" else "#334155"
                                st.markdown(f'''<div class="card" style="border-color:{bc}">
                                <b style="color:{bc}">{"✅ " if meill=="Bareme" else ""}Barème {tmi} % + abatt. 40 %</b><br>
                                IR {fr(bar.get("ir",0))} € + PS {fr(bar.get("ps",0))} € = <b>{fr(bar["total_imposition"])} €</b><br>
                                Net perçu : <b>{fr(bar["net_percu"])} €</b></div>''', unsafe_allow_html=True)
                            st.caption(f"Économie option recommandée : {fr(eco)} €")
                        if 'sim_rem_div' in r:
                            sim = r['sim_rem_div']
                            with st.expander("💡 Simulation rémunération vs dividendes"):
                                cc1, cc2 = st.columns(2)
                                with cc1:
                                    sa = sim['strategie_a']
                                    st.markdown(f"**{sa['label']}**")
                                    st.metric("Coût total (IS + IR)", f"{fr(sa['cout_total'])} €")
                                    st.caption(f"Net perçu : {fr(sa['net_percu'])} €")
                                with cc2:
                                    sb = sim['strategie_b']
                                    st.markdown(f"**{sb['label']}**")
                                    st.metric("Coût total (IS + PFU)", f"{fr(sb['cout_total'])} €")
                                    st.caption(f"Net perçu : {fr(sb['net_percu'])} €")
                                st.success(f"✅ **{sim['meilleure']} recommandée** — Économie : {fr(sim.get('economie',0))} €")
                        if 'optimisations' in r and r['optimisations']:
                            ic_map = {"Tres eleve":"#f59e0b","Eleve":"#3b82f6","A evaluer":"#8b5cf6","Moyen":"#6b7280"}
                            with st.expander(f"💡 {len(r['optimisations'])} recommandations fiscales"):
                                for opt in r['optimisations'][:5]:
                                    ic = ic_map.get(opt['impact'], "#6b7280")
                                    gs = f" — Gain : {fr(opt['gain_estime'])} €" if opt.get('gain_estime') else ""
                                    ph = "".join(f"<div style='color:#4ade80;font-size:.82rem'>+ {p}</div>" for p in opt.get('pour',[]))
                                    ch = "".join(f"<div style='color:#f87171;font-size:.82rem'>- {c}</div>" for c in opt.get('contre',[]))
                                    st.markdown(
                                        f"<div style='background:#1e293b;border-left:4px solid {ic};"
                                        f"border-radius:0 8px 8px 0;padding:.8rem;margin:.4rem 0'>"
                                        f"<div style='color:{ic};font-weight:600'>{opt['titre']}"
                                        f"<span style='background:{ic}22;font-size:.73rem;padding:.15rem .5rem;"
                                        f"border-radius:12px;margin-left:.5rem'>{opt['impact']}</span>{gs}</div>"
                                        f"<div style='color:#94a3b8;font-size:.83rem;margin:.3rem 0'>{opt['detail']}</div>"
                                        f"<div style='color:#60a5fa;font-size:.83rem'>→ {opt['action']}</div>"
                                        f"<div style='display:flex;gap:2rem;margin-top:.3rem'>{ph}{ch}</div></div>",
                                        unsafe_allow_html=True
                                    )
                        if len(ents_res) > 1:
                            st.markdown("---")

            autres_res = res.get('autres_revenus', {})
            if autres_res:
                st.markdown("### 🏘️ Revenus complémentaires")
                for ak, ar in autres_res.items():
                    lbl  = _AUTRES_REV_DIR.get(ak, {}).get('label', ak)
                    calc = ar.get('calcul', {})
                    imp_a = calc.get('imposable', calc.get('benefice_imposable', 0))
                    with st.expander(f"📋 {lbl}", expanded=False):
                        if 'erreur' in ar:
                            st.error(f"Erreur : {ar['erreur']}")
                        else:
                            cc1, cc2 = st.columns(2)
                            cc1.metric("Imposable / Résultat", f"{fr(imp_a)} €")
                            cc2.metric(f"IR estimé (TMI {tmi}%)", f"{fr(int(imp_a * tmi / 100))} €")
                            if calc.get('note'):
                                st.info(calc['note'])
            ...
            c1, _, c2 = st.columns([1, 3, 1])
            with c1:
                if st.button("← Modifier les données", key="dprev_4"):
                    prev_step()
            with c2:
                if st.button("📄 Rapport PDF →", key="dnext_4", type="primary"):
                    next_step()
    ...
    # ════════════════════════════════════════════════════════════
        ...
        st.markdown("## 📄 Rapport PDF — Bilan fiscal dirigeant")
        res = SS.dir_resultats
        tmi = d.get('tmi_dir', 30)
        if not res:
            st.warning("Résultats manquants. Retournez calculer vos résultats.")
            if st.button("← Retour", key="dprev_5_nores"):
                prev_step()
        else:
            total_imp = res.get('total_imposable', 0)
            total_cot = res.get('total_cotisations', 0)
            ...
            with c1:
                st.markdown(kpi_html("Revenus imposables IR", f"{fr(total_imp)} €"), unsafe_allow_html=True)
            with c2:
                st.markdown(kpi_html("Cotisations estimées",  f"{fr(total_cot)} €", "#f59e0b"), unsafe_allow_html=True)
            with c3:
                st.markdown(kpi_html("IR estimé (TMI)",       f"{fr(int(total_imp*tmi/100))} €", "#f87171"), unsafe_allow_html=True)
            st.markdown("---")
            if st.button("📥 Générer et télécharger le rapport PDF", type="primary", key="dir_pdf_gen"):
                with st.spinner("Génération en cours..."):
                    try:
                        from rapport_pdf import GenererRapportDirigeantPDF
                        profil_dir = {
                            'statuts':        SS.dir_statuts,
                            'statuts_labels': [_STATUTS_DIR[sk]['label'] for sk in SS.dir_statuts],
                            'donnees':        SS.dir_donnees,
                            'nb_ent':         SS.dir_nb_ent,
                            'tmi':            tmi,
                            'autres_actifs':  SS.dir_autres_actifs,
                            'autres_donnees': SS.dir_autres_donnees,
                            'statut':         SS.dir_statuts[0] if SS.dir_statuts else '',
                            'remuneration':   sum(e.get('remuneration',0) for sk in SS.dir_statuts for e in SS.dir_donnees.get(sk,[])),
                            'benefice':       sum(e.get('benefice',0)     for sk in SS.dir_statuts for e in SS.dir_donnees.get(sk,[])),
                            'dividendes':     sum(e.get('dividendes',0)+e.get('dividendes_sci',0) for sk in SS.dir_statuts for e in SS.dir_donnees.get(sk,[])),
                        }
                        gen_d = GenererRapportDirigeantPDF()
                        if hasattr(gen_d, 'generer_multistatut'):
                            pdf_bytes = gen_d.generer_multistatut(profil_dir, res, dm)
                        else:
                            pdf_bytes = gen_d.generer(profil_dir, dm)
                        b64  = base64.b64encode(pdf_bytes).decode()
                        href = (
                            f'<a href="data:application/pdf;base64,{b64}" '
                            f'download="bilan_fiscal_dirigeant_2026.pdf" '
                            f'style="display:inline-block;background:#1d4ed8;color:white;'
                            f'padding:.8rem 2rem;border-radius:8px;text-decoration:none;'
                            f'font-weight:700;margin-top:.8rem">'
                            f'⬇️ Télécharger le rapport PDF dirigeant</a>'
                        )
                        st.markdown(href, unsafe_allow_html=True)
            ...
            c1, _, c2 = st.columns([1, 3, 1])
            with c1:
                if st.button("← Retour aux résultats", key="dprev_5"):
                    prev_step()
            with c2:
                if st.button("🔄 Nouvelle simulation", key="dir_reset_5"):
                    for _k in ['dir_statuts','dir_nb_ent','dir_donnees',
                               'dir_autres_actifs','dir_autres_donnees','dir_resultats']:
                        if _k in SS: del SS[_k]
                    go_step(0)
