...
# ═══════════════════════════════════════════════════════════════════
if SS.step == 0:
    st.markdown("## Bonjour ! Pour commencer, que souhaitez-vous simuler ?")
    st.markdown("Cette simulation vous guidera etape par etape, en quelques minutes.")
    st.markdown("")

    col1, col2, col3 = st.columns(3)
    with col1:
        st.markdown("""<div class="card" style="min-height:160px">
        <h4>Mon foyer fiscal</h4>
        <p style="color:#94a3b8;font-size:.88rem">Simulation complete de votre impot : revenus, deductions, credits, optimisations.</p>
        </div>""", unsafe_allow_html=True)
        if st.button("Simuler mon foyer →", key="btn_foyer"):
            SS.mode = "foyer"; SS.data = {}; next_step()

    with col2:
        st.markdown("""<div class="card" style="min-height:160px">
        <h4>Comparaison enfant majeur</h4>
        <p style="color:#94a3b8;font-size:.88rem">Rattachement vs foyer independant : comparez les deux scenarios pour faire le meilleur choix.</p>
        </div>""", unsafe_allow_html=True)
        if st.button("Comparer les scenarios →", key="btn_comp"):
            SS.mode = "comparaison"; SS.data = {}; next_step()

    with col3:
        st.markdown("""<div class="card" style="min-height:160px">
        <h4>Revenus de dirigeant</h4>
        <p style="color:#94a3b8;font-size:.88rem">BIC, BNC, SCI, LMNP, LMP, IS, dividendes, TNS... Optimisez votre remuneration.</p>
        </div>""", unsafe_allow_html=True)
        if st.button("Simuler mes revenus pro →", key="btn_dir"):
            SS.mode = "dirigeant"; SS.data = {}; next_step()

    st.markdown("")
    st.markdown('<div class="src">Simulation a titre informatif — Brochure DGFiP 2025 (revenus 2024) — Consultez impots.gouv.fr ou un expert-comptable pour votre situation.</div>',
                unsafe_allow_html=True)

# ═══════════════════════════════════════════════════════════════════
# WIZARD FOYER / COMPARAISON
# ═══════════════════════════════════════════════════════════════════
