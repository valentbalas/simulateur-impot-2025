...
.stButton>button{background:#1d4ed8;color:white;border:none;
  border-radius:8px;font-weight:600;padding:.55rem 1.4rem;width:100%}
.stButton>button:hover{background:#2563eb}
.btn-nav{display:flex;gap:.8rem;margin-top:1.5rem}
/* Progress steps */
...
.step-dot.done{background:#1d4ed8;color:white}
.step-dot.current{background:#3b82f6;color:white;box-shadow:0 0 0 3px #1e3a5f}
.step-dot.todo{background:#1e293b;color:#475569;border:1px solid #334155}
.step-label{font-size:11px;color:#94a3b8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:80px}
.step-label.current{color:#93c5fd;font-weight:600}
.step-sep{width:24px;height:1px;background:#334155;flex-shrink:0;margin:0 4px}
/* Cards */
.card{background:#1e293b;border:1px solid #334155;border-radius:12px;padding:1.3rem;margin:.5rem 0}
.card h4{color:#93c5fd;margin:0 0 .7rem;font-size:1rem}
...
.verdict-good{background:#14532d;border-left:4px solid #22c55e;
  border-radius:0 8px 8px 0;padding:1rem;margin:.7rem 0}
.verdict-neutral{background:#1e293b;border-left:4px solid #3b82f6;
  border-radius:0 8px 8px 0;padding:1rem;margin:.7rem 0}
.conseil{background:#1c1a09;border:1px solid #854d0e;border-radius:8px;
  padding:.8rem;margin:.35rem 0}
.src{background:#1e293b;border-radius:6px;padding:.4rem .8rem;
  font-size:.73rem;color:#64748b;margin:.4rem 0}
.highlight{background:#1e3a5f;border:1px solid #1d4ed8;border-radius:8px;padding:.8rem 1rem;margin:.3rem 0}
/* Frais reels */
.fr-row{display:grid;grid-template-columns:1fr 1fr;gap:.5rem;margin:.3rem 0}
.fr-item{background:#1e293b;border-radius:6px;padding:.5rem .8rem;border:1px solid #334155}
.fr-item .lbl{font-size:.75rem;color:#94a3b8}
.fr-item .val{font-size:.95rem;font-weight:600;color:#e2e8f0}
.compare-bar{background:#1e293b;border-radius:8px;padding:1rem;margin:.5rem 0;border:1px solid #334155}
/* Fix: no raw code visible */
...
# ═══════════════════════════════════════════════════════════════════
    ...
    # ── Étape 2 : Revenus Déclarant 1 ─────────────────────────────
        ...
        st.info(
            ...
            "Les indemnités journalières (IJ) versées par la CPAM sont **imposables** et normalement "
            "pré-remplies dans votre case 1AJ sur impots.gouv.fr — **incluez-les dans le montant ci-dessous**. "
            "⚠️ Exception : les IJ pour **ALD (maladie longue durée)** sont **exonérées** — ne pas les inclure."
        )
        c1, c2 = st.columns(2)
        with c1:
            sal1 = st.number_input("Salaires / Traitements EUR (case 1AJ — incl. IJ CPAM si arrêt maladie ordinaire)",
                                   0, 500000, int(d.get('sal1', 45000)), 500, key="sal1_w",
                                   help="Inclut vos salaires + IJ maladie ordinaire, maternité, paternité. "
                                        "Excluez les IJ ALD (exonérées).")
